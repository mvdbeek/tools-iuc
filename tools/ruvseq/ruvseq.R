...
run_deseq_on_set <- function(set, dds) {
  k = sum(sapply(colnames(pData(set)), FUN=startsWith, prefix='W_'))
  ddsruv <- dds
  design_formula <- "~ "
  if (k > 0) {
    for (k_ in seq(k)) {
      current_attr = paste0('W_', k_)
      ddsruv[[current_attr]] <- set[[current_attr]]
      design_formula <- paste0(design_formula, '+ ', current_attr, ' ')
    }
    design_formula <- paste0(design_formula, ' + condition')
  } else {
    design_formula <- paste0(design_formula, 'condition')
  }
  design(ddsruv) <- as.formula(design_formula)
  ddsruv <- DESeq(ddsruv)
  return(ddsruv)
}
